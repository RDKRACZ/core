...
#define initObj(t) type = t
    ...
    public:
        ...
        GBool isName(const char *nameA) { return type == objName && !strcmp(name, nameA); }

        GBool isDict(const char *dictType);

        GBool isStream(char *dictType);

        GBool isCmd(const char *cmdA) { return type == objCmd && !strcmp(cmd, cmdA); }
...
namespace PdfReader {
    ...
    inline Object *Object::arrayGet(int i, Object *obj, int recursion) { return array->get(i, obj, recursion); }

    inline Object *Object::arrayGetNF(int i, Object *obj) { return array->getNF(i, obj); }
...
namespace PdfReader {
    ...
    inline GBool Object::dictIs(const char *dictType) { return dict->is(dictType); }

    inline GBool Object::isDict(const char *dictType) { return type == objDict && dictIs(dictType); }

    inline Object *Object::dictLookup(const char *key, Object *obj, int recursion) {
        return dict->lookup(key, obj, recursion);
    }

    inline Object *Object::dictLookupNF(const char *key, Object *obj) { return dict->lookupNF(key, obj); }

    inline char *Object::dictGetKey(int i) { return dict->getKey(i); }

    inline Object *Object::dictGetVal(int i, Object *obj) { return dict->getVal(i, obj); }

    inline Object *Object::dictGetValNF(int i, Object *obj) { return dict->getValNF(i, obj); }
}
//------------------------------------------------------------------------
// Stream accessors.
//------------------------------------------------------------------------
...
#include "Stream.h"
namespace PdfReader {
    inline GBool Object::streamIs(char *dictType) { return stream->getDict()->is(dictType); }

    inline GBool Object::isStream(char *dictType) { return type == objStream && streamIs(dictType); }
    ...
    inline int Object::streamGetBlock(char *blk, int size) { return stream->getBlock(blk, size); }

    inline char *Object::streamGetLine(char *buf, int size) { return stream->getLine(buf, size); }

    inline GFileOffset Object::streamGetPos() { return stream->getPos(); }

    inline void Object::streamSetPos(GFileOffset pos, int dir) { stream->setPos(pos, dir); }
