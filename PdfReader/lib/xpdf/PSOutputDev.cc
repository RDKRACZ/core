...
//------------------------------------------------------------------------
    ...
    public:
        ...
        virtual int lookChar() { return (bufIdx >= bufSize && !fillBuf()) ? EOF : buf[bufIdx]; }

        virtual GString *getPSFilter(int psLevel, const char *indent,
                                     GBool okToReadStream) { return NULL; }
...
//------------------------------------------------------------------------
    ...
    public:
        ...
        virtual int getChar() { return (bufIdx >= width && !fillBuf()) ? EOF : buf[bufIdx++]; }

        virtual int lookChar() { return (bufIdx >= width && !fillBuf()) ? EOF : buf[bufIdx]; }
...
//~ this doesn't currently support OPI
    ...
    void PSOutputDev::dumpLabColorSpace(GfxLabColorSpace *cs,
        ...
        if (map01) {
            ...
            writePSFmt(
                    " /DecodeABC [{{100 mul 16 add 116 div}} bind {{{0:.4g} mul {1:.4g} add}} bind {{{2:.4g} mul {3:.4g} add}} bind]\n",
            ...
            case 2:            // exponential
                ...
                for (i = 0; i < n; ++i) {
                    ...
                    if (func2->getHasRange()) {
                        writePSFmt(
                                "dup {0:.6g} lt {{ pop {1:.6g} }} {{ dup {2:.6g} gt {{ pop {3:.6g} }} if }} ifelse\n",
