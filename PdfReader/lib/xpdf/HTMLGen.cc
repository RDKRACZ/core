...
//------------------------------------------------------------------------
    ...
    public:
        ...
        GBool match(Ref fontIDA) { return fontIDA.num == fontID.num && fontIDA.gen == fontID.gen; }
    ...
    int HTMLGen::convertPage(
        ...
        if (primaryDir >= 0) {
            pf(writeHTML, htmlStream,
               "<img id=\"background\" style=\"position:absolute; left:0px; top:0px;\" width=\"{0:d}\" height=\"{1:d}\" src=\"{2:s}\">\n",
               (int) (pageW * zoom), (int) (pageH * zoom), pngURL);
        } else {
            pf(writeHTML, htmlStream,
               "<img id=\"background\" style=\"position:absolute; right:0px; top:0px;\" width=\"{0:d}\" height=\"{1:d}\" src=\"{2:s}\">\n",
               (int) (pageW * zoom), (int) (pageH * zoom), pngURL);
        }
        ...
        for (colIdx = 0; colIdx < cols->getLength(); ++colIdx) {
            ...
            for (parIdx = 0; parIdx < pars->getLength(); ++parIdx) {
                ...
                for (lineIdx = 0; lineIdx < lines->getLength(); ++lineIdx) {
                    ...
                    if (primaryDir >= 0) {
                        pf(writeHTML, htmlStream,
                           "<div class=\"txt\" style=\"position:absolute; left:{0:d}px; top:{1:d}px;\">{2:t}</div>\n",
                           (int) (line->getXMin() * zoom),
                           (int) (line->getYMin() * zoom), s);
                    } else {
                        pf(writeHTML, htmlStream,
                           "<div class=\"txt\" style=\"position:absolute; right:{0:d}px; top:{1:d}px;\">{2:t}</div>\n",
    ...
    void HTMLGen::appendSpans(GList *words, int firstWordIdx, int lastWordIdx,
        ...
        for (wordIdx = firstWordIdx;
            ...
            if (!word0 ||
                ...
                s->appendf(
                        "<span id=\"f{0:d}\"{1:s} style=\"font-size:{2:d}px;vertical-align:{3:s};{4:s}color:rgba({5:d},{6:d},{7:d},{8:d});\">",
                        i,
                        dirTag,
